...
pretty_map <- c(
  ...
  "high"              = "high",
  "very high"         = "very high",
  "weight_prior_true" = "From the data - weighting",
  "weight_prior_false"= "From the data - no weighting"
)

order_levels <- c("no delay", "correct",
                  "weight_prior_true", "weight_prior_false")
...
plot_cs_triple <- function(res_samples,
  ...
  rankings <- rankings |>
    mutate(
      inc_period = factor(
        inc_period, levels=order_levels
      ),
      gen_time = factor(
        gen_time, levels=order_levels
      ))
 ...
 all_dates <- expand_grid(
  ...
  scores_cases_long$gen_time <- factor(scores_cases_long$gen_time, levels=order_levels)
  
  scores_cases_long$inc_period <- factor(scores_cases_long$inc_period, levels=order_levels)
  ...
  barchart_gen_time <- ggplot(scores_cases_gen_time |> filter(measure!="crps")) + 
    ...
    scale_x_discrete(limits = order_levels, labels = pretty_map) + 
...
plot_cs_compact <- function(res_samples,
                           sim_data,
                           forecast_freq=4,
                           data_freq="day") {
...
plot_cs_single <- function(res_samples,
  ...
  return(list(timeseries = timeseries_forecast,
              barchart_gen_time = barchart_mean_gen_time,
              barchart_inc_period = barchart_mean_inc_period,
              combined_legends = combined_legends))
}
