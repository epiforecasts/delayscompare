install.packages("EpiNow2", repos="http://cran.ma.imperial.ac.uk/", type="binary")
...
sim_scenarios <- function(case_data,
  ...
  for(i in 1:length(scen_values)){
    for(j in 1:length(scen_values)){
      ...
      # Incubation period
      inc_period <- dist_spec(mean=convert_to_logmean(inc_mean*scen_values[j], inc_sd),
                              sd=convert_to_logsd(inc_mean*scen_values[j], inc_sd),
                              max=inc_max,
